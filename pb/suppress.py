...
def get_best_gridpoints(gps_start,
  ...
  for starttime in range(int(stime.gps), int(stime.gps + duration), int(step)):
    ...
    logger.debug("Number of gridpoints skipped due to gain lower than minimum (=%.2f) = %d" % (min_gain, skipped_gain_too_low))
    ...
    if best_gridpoint is not None:
      ...
      logger.debug(outstring % (gp_numbers[best_gridpoint], gp_azes[best_gridpoint], gp_alts[best_gridpoint], t.iso, r_max, best_gain_obs, best_gain_avoid))
