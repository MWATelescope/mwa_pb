  ...
  def get_FF(self, phi_arr, theta_arr, grid):
    ...
    if grid:
      ...
      if phi_arr.ndim != 1 and theta_arr.ndim != 1:
        e = 'For gridded beam, theta (shape %s) and phi (shape %s) must be 1-D arrays' % (np.shape(theta_arr), np.shape(phi_arr))
        logger.error(e)
        raise ValueError(e)
      ...
      if phi_arr.shape != theta_arr.shape:
        e = 'Theta (shape %s) and phi (shape %s) must be the same shape' % (np.shape(theta_arr), np.shape(phi_arr))
        logger.error(e)
        raise ValueError(e)
      Jones = np.zeros((2, 2) + np.shape(phi_arr), dtype=np.complex128)
