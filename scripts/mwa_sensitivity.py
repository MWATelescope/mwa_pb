...
def calculate_sensitivity(freq, delays, gps, trcv_type, T_rcv, size, dirname, model, plottype, extension,
                          pointing_az_deg=0, pointing_za_deg=0, add_sources=False, zenithnorm=True, antnum=128,
                          inttime=120, bandwidth=1280000):
  freq_mhz = freq / 1e6
  ...
  (beamsky_sum_XX, beam_sum_XX, Tant_XX, beam_dOMEGA_sum_XX, beamsky_sum_YY, beam_sum_YY, Tant_YY, beam_dOMEGA_sum_YY) = result

  beams = get_beam_power(gps, delays, freq, model=model, pointing_az_deg=pointing_az_deg,
                         pointing_za_deg=pointing_za_deg, zenithnorm=zenithnorm)
...
#  datetimestring = options.datetimestring
  ...
  print "Pointing direction to source = (az,za) = (%.8f,%.8f) [deg]" % (options.pointing_az_deg, options.pointing_za_deg)
